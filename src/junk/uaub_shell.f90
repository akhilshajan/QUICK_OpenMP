subroutine get2e_uab(II_arg, flag_uscf)
   ...
   double precision :: flag_uscf ! Flag to indicate if scf (0) vs uscf (1) calculation 
...
!           II, JJ, KK, LL !Madu            
                  call shell_uab(flag_uscf)
           endif
         enddo
      enddo 
   enddo
end subroutine get2e_uab


! Vertical Recursion by Xiao HE 07/07/07 version
subroutine shell_uab(flag_uscf)
...
!  stop
!--------------------Madu--------------------------
               call iclass_uab(I,J,K,L,NNA,NNC,NNAB,NNCD,flag_uscf)
            enddo
         enddo
      enddo
   enddo
   201 return
end subroutine shell_uab

! Horrizontal recursion and Fock matrix builder by Xiao HE 07/07/07 version
subroutine iclass_uab(I,J,K,L,NNA,NNC,NNAB,NNCD,flag_uscf)
   use allmod
            ...
            do JJJ=JJJ1,JJJ2
               do KKK=KKK1,KKK2
                  do LLL=LLL1,LLL2
                     ...
                     DENSELK=quick_qm_struct%dense(LLL,KKK)+flag_uscf*quick_qm_struct%denseb(LLL,KKK)
                     DENSEJI=quick_qm_struct%dense(JJJ,III)+flag_uscf*quick_qm_struct%denseb(JJJ,III)
                     ...
                     DENSEKIB=flag_uscf*quick_qm_struct%denseb(KKK,III)
                     DENSEKJB=flag_uscf*quick_qm_struct%denseb(KKK,JJJ)
                     DENSELJB=flag_uscf*quick_qm_struct%denseb(LLL,JJJ)
                     DENSELIB=flag_uscf*quick_qm_struct%denseb(LLL,III)
                     ...
                     quick_qm_struct%o(KKK,III) = quick_qm_struct%o(KKK,III)-quick_method%x_hybrid_coeff* &
                                                  (0.5d0+0.5d0*flag_uscf)*DENSELJA*Y
                     quick_qm_struct%o(LLL,III) = quick_qm_struct%o(LLL,III)-quick_method%x_hybrid_coeff* &
                                                  (0.5d0+0.5d0*flag_uscf)*DENSEKJA*Y
                     quick_qm_struct%o(JJJ,KKK) = quick_qm_struct%o(JJJ,KKK)-quick_method%x_hybrid_coeff* &
                                                  (0.5d0+0.5d0*flag_uscf)*DENSELIA*Y
                     quick_qm_struct%o(JJJ,LLL) = quick_qm_struct%o(JJJ,LLL)-quick_method%x_hybrid_coeff* &
                                                  (0.5d0+0.5d0*flag_uscf)*DENSEKIA*Y
                     quick_qm_struct%o(KKK,JJJ) = quick_qm_struct%o(KKK,JJJ)-quick_method%x_hybrid_coeff* &
                                                  (0.5d0+0.5d0*flag_uscf)*DENSELIA*Y
                     quick_qm_struct%o(LLL,JJJ) = quick_qm_struct%o(LLL,JJJ)-quick_method%x_hybrid_coeff* &
                                                  (0.5d0+0.5d0*flag_uscf)*DENSEKIA*Y

                     quick_qm_struct%ob(JJJ,III) = quick_qm_struct%ob(JJJ,III)+2.d0*flag_uscf*DENSELK*Y
                     quick_qm_struct%ob(LLL,KKK) = quick_qm_struct%ob(LLL,KKK)+2.d0*flag_uscf*DENSEJI*Y
...
!--------------------Madu--------------------------
                           ...
                           DENSEJJ=quick_qm_struct%dense(KKK,KKK)+flag_uscf*quick_qm_struct%denseb(KKK,KKK)
                           DENSEII=quick_qm_struct%dense(III,III)+flag_uscf*quick_qm_struct%denseb(III,III)

                           DENSEJIA=quick_qm_struct%dense(KKK,III)
                           DENSEJIB=flag_uscf*quick_qm_struct%denseb(KKK,III)
                           ...
                           quick_qm_struct%o(KKK,III) = quick_qm_struct%o(KKK,III)-quick_method%x_hybrid_coeff* &
                                                        (0.5d0+0.5d0*flag_uscf)*DENSEJIA*Y

                           quick_qm_struct%ob(III,III) = quick_qm_struct%ob(III,III)+flag_uscf*DENSEJJ*Y
                           quick_qm_struct%ob(KKK,KKK) = quick_qm_struct%ob(KKK,KKK)+flag_uscf*DENSEII*Y
...
!--------------------Madu--------------------------
                           ...
                           DENSEJI=quick_qm_struct%dense(JJJ,III)+flag_uscf*quick_qm_struct%denseb(JJJ,III)
                           DENSEJJ=quick_qm_struct%dense(JJJ,JJJ)+flag_uscf*quick_qm_struct%denseb(JJJ,JJJ)

                           DENSEJIA=quick_qm_struct%dense(JJJ,III)
                           DENSEJJA=quick_qm_struct%dense(JJJ,JJJ)

                           DENSEJIB=flag_uscf*quick_qm_struct%denseb(JJJ,III)
                           DENSEJJB=flag_uscf*quick_qm_struct%denseb(JJJ,JJJ)

                           ! Find  all the (ij|jj) integrals.
                           quick_qm_struct%o(JJJ,III) = quick_qm_struct%o(JJJ,III)+DENSEJJ*Y &
                                -quick_method%x_hybrid_coeff*(0.5d0+0.5d0*flag_uscf)*DENSEJJA*Y
                           quick_qm_struct%o(JJJ,JJJ) = quick_qm_struct%o(JJJ,JJJ)+2.0d0*DENSEJI*Y &
                                -quick_method%x_hybrid_coeff*(1.0d0+1.0d0*flag_uscf)*DENSEJIA*Y 

                           quick_qm_struct%ob(JJJ,III) = quick_qm_struct%ob(JJJ,III)+flag_uscf*DENSEJJ*Y &
                                -quick_method%x_hybrid_coeff*DENSEJJB*Y
                           quick_qm_struct%ob(JJJ,JJJ) = quick_qm_struct%ob(JJJ,JJJ)+2.0d0*flag_uscf*DENSEJI*Y &
...
!--------------------Madu--------------------------
                           ...
                           DENSEKK=quick_qm_struct%dense(KKK,KKK)+flag_uscf*quick_qm_struct%denseb(KKK,KKK)
                           DENSEJI=quick_qm_struct%dense(JJJ,III)+flag_uscf*quick_qm_struct%denseb(JJJ,III)
                           ...
                           quick_qm_struct%ob(JJJ,III) = quick_qm_struct%ob(JJJ,III)+flag_uscf*DENSEKK*Y
                           quick_qm_struct%ob(KKK,KKK) = quick_qm_struct%ob(KKK,KKK)+2.d0*flag_uscf*DENSEJI*Y
...
!--------------------Madu--------------------------
                           ...
                           DENSEII=quick_qm_struct%dense(III,III)+flag_uscf*quick_qm_struct%denseb(III,III)
                           DENSEKJ=quick_qm_struct%dense(LLL,KKK)+flag_uscf*quick_qm_struct%denseb(LLL,KKK)

                           DENSEKIA=quick_qm_struct%dense(LLL,III)
                           DENSEJIA=quick_qm_struct%dense(KKK,III)                          

                           DENSEKIB=flag_uscf*quick_qm_struct%denseb(LLL,III)
                           DENSEJIB=flag_uscf*quick_qm_struct%denseb(KKK,III)
                           ...
                           quick_qm_struct%ob(LLL,KKK) = quick_qm_struct%ob(LLL,KKK)+flag_uscf*DENSEII*Y
                           quick_qm_struct%ob(III,III) = quick_qm_struct%ob(III,III)+2.d0*flag_uscf*DENSEKJ*Y
...
!--------------------Madu--------------------------
                        ...
                        if(JJJ.LE.LLL)then
                           ...
                           if(III.eq.JJJ.and.III.eq.KKK.and.III.eq.LLL)then
                              ...
                              DENSEIIB=flag_uscf*quick_qm_struct%denseb(III,III)

                              ! do all the (ii|ii) integrals.
                              quick_qm_struct%o(III,III) = quick_qm_struct%o(III,III)+DENSEII*Y-quick_method%x_hybrid_coeff* &
                                                           (0.5d0+0.5d0*flag_uscf)*DENSEIIA*Y

                              quick_qm_struct%ob(III,III) = quick_qm_struct%ob(III,III)+flag_uscf*DENSEII*Y- &
...
!--------------------Madu--------------------------
                              ...
                              DENSEJI=quick_qm_struct%dense(LLL,III)+flag_uscf*quick_qm_struct%denseb(LLL,III)
                              DENSEII=quick_qm_struct%dense(III,III)+flag_uscf*quick_qm_struct%denseb(III,III)

                              DENSEJIA=quick_qm_struct%dense(LLL,III)
                              DENSEIIA=quick_qm_struct%dense(III,III)

                              DENSEJIB=flag_uscf*quick_qm_struct%denseb(LLL,III)
                              DENSEIIB=flag_uscf*quick_qm_struct%denseb(III,III)                              

                              ! Find  all the (ii|ij) integrals.
                              quick_qm_struct%o(LLL,III) = quick_qm_struct%o(LLL,III)+DENSEII*Y &
                                -quick_method%x_hybrid_coeff*(0.5d0+0.5d0*flag_uscf)*DENSEIIA*Y
                              quick_qm_struct%o(III,III) = quick_qm_struct%o(III,III)+2.0d0*DENSEJI*Y &
                                -quick_method%x_hybrid_coeff*(1.0d0+1.0d0*flag_uscf)*DENSEJIA*Y 

                              quick_qm_struct%ob(LLL,III) = quick_qm_struct%ob(LLL,III)+flag_uscf*DENSEII*Y &
                                -quick_method%x_hybrid_coeff*DENSEIIB*Y
                              quick_qm_struct%ob(III,III) = quick_qm_struct%ob(III,III)+2.0d0*flag_uscf*DENSEJI*Y &
...
!--------------------Madu--------------------------
                             ...
                             ! Find all the (ij|ij) integrals
                              quick_qm_struct%o(JJJ,III) = quick_qm_struct%o(JJJ,III)+2.0d0*DENSEJI*Y &
                                                           -quick_method%x_hybrid_coeff*(0.5d0+0.5d0*flag_uscf)*DENSEJIA*Y
                              ...
                              quick_qm_struct%o(III,III) = quick_qm_struct%o(III,III)-quick_method%x_hybrid_coeff* &
                                                           (0.5d0+0.5d0*flag_uscf)*DENSEJJA*Y

                              quick_qm_struct%ob(JJJ,III) = quick_qm_struct%ob(JJJ,III)+2.0d0*flag_uscf*DENSEJI*Y &
...
!--------------------Madu--------------------------
                              ...
                              DENSEKI=quick_qm_struct%dense(LLL,III)+flag_uscf*quick_qm_struct%denseb(LLL,III)
                              DENSEJI=quick_qm_struct%dense(JJJ,III)+flag_uscf*quick_qm_struct%denseb(JJJ,III)
                              ...
                              DENSEKIB=flag_uscf*quick_qm_struct%denseb(LLL,III)
                              DENSEKJB=flag_uscf*quick_qm_struct%denseb(LLL,JJJ)
                              DENSEIIB=flag_uscf*quick_qm_struct%denseb(III,III)
                              DENSEJIB=flag_uscf*quick_qm_struct%denseb(JJJ,III)

                              ! Find all the (ij|ik) integrals where j>i,k>j
                              quick_qm_struct%o(JJJ,III) = quick_qm_struct%o(JJJ,III)+2.0d0*DENSEKI*Y &
                                -quick_method%x_hybrid_coeff*(0.5d0+0.5d0*flag_uscf)*DENSEKIA*Y
                              quick_qm_struct%o(LLL,III) = quick_qm_struct%o(LLL,III)+2.0d0*DENSEJI*Y &
                                - quick_method%x_hybrid_coeff*(0.5d0+0.5d0*flag_uscf)*DENSEJIA*Y
                              quick_qm_struct%o(III,III) = quick_qm_struct%o(III,III)-quick_method%x_hybrid_coeff* &
                                                           (1.d0+1.0d0*flag_uscf)*DENSEKJA*Y 
                              quick_qm_struct%o(LLL,JJJ) = quick_qm_struct%o(LLL,JJJ)-quick_method%x_hybrid_coeff* &
                                                            (0.5d0+0.5d0*flag_uscf)*DENSEIIA*Y

                              quick_qm_struct%ob(JJJ,III) = quick_qm_struct%ob(JJJ,III)+2.0d0*flag_uscf*DENSEKI*Y &
                                                            -quick_method%x_hybrid_coeff*DENSEKIB*Y
                              quick_qm_struct%ob(LLL,III) = quick_qm_struct%ob(LLL,III)+2.0d0*flag_uscf*DENSEJI*Y &
                                                            - quick_method%x_hybrid_coeff*DENSEJIB*Y
...
!--------------------Madu--------------------------
   ...
   201 return
End subroutine iclass_uab

!------------------------------------------------
! get2eEnergy
!------------------------------------------------
subroutine get2eEnergy_new_imp(flag_uscf)
   use allmod
   implicit double precision(a-h,o-z)
   double precision :: flag_uscf
   ...
   quick_qm_struct%Eel=quick_qm_struct%Eel+Sum2Mat(quick_qm_struct%dense,quick_qm_struct%o,nbasis)+ &
                       flag_uscf*Sum2Mat(quick_qm_struct%denseb,quick_qm_struct%ob,nbasis)
   quick_qm_struct%Eel=quick_qm_struct%Eel/2.0d0
   call cpu_time(timer_end%TE)  ! Terminate the timer for energy
   timer_cumer%TE=timer_cumer%TE+timer_end%TE-timer_begin%TE ! add the time to
end subroutine get2eEnergy_new_imp
